...
const Skills = () => {
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-center mb-8 opacity-0 animate-fadeInUp relative z-10 px-4" style={{ animationDelay: "0.2s" }}>
        <div className="inline-flex bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-full p-1 flex-wrap gap-1">
          {skillCategories.map((category) => (
            <button
              key={category.id}
              onClick={() => setActiveCategory(category.id)}
              className={`px-4 md:px-6 py-2 md:py-3 rounded-full font-medium transition-all duration-300 text-sm md:text-base ${activeCategory === category.id
